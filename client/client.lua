...
CreateThread(function()
    local pedModel = joaat(Config.PedModel)

    lib.requestModel(pedModel, 15000)
...
RegisterNetEvent('qb-carboosting:client:startBoostingRequest', function()
    if not boosting then
        ...
        JobEmail(
            'Yo,<br /><br />You have a new vehicle order. Please accept or decline the request on your phone.<br />',
            'qb-carboosting:client:acceptBoostingMission')
    else
...
RegisterNetEvent('qb-carboosting:client:startBoosting', function()
    if not boosting then
        ...
        local vehicleHash = joaat(targetCar)

        lib.requestModel(vehicleHash, 15000)
